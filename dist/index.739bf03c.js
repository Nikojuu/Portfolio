...
},{"./observers":"dR1N6"}],"dR1N6":[function(require,module,exports) {
...
const removeSkillsEffect = function() {
    skills.forEach((skill)=>skill.classList.remove("card__skills__skill--active"));
};
...
projectsImg.forEach((img)=>{
    const imgObserver = new IntersectionObserver((entries)=>{
        entries.forEach(async (entry)=>{
            if (!entry.isIntersecting) return;
            if (entry.isIntersecting && entry.target.classList.contains("projects__image-2")) {
                projectsDescription[1].style.animation = "slideIn-left 1.5s cubic-bezier(0.6, -0.05, 0.1, 0.99) both";
                verticalLineRight.classList.add("vertical__line");
                horizontalLineRight.classList.add("horizontal__line");
                imgObserver.unobserve(entry.target);
            } else if (entry.isIntersecting && entry.target.classList.contains("projects__image-1")) {
                ...
                projectsDescription[0].style.animation = "slideIn-right 1.5s cubic-bezier(0.6, -0.05, 0.1, 0.99) both";
                await waitTime(1000);
